...
echo "InGenioColombia:$PASS" | chpasswd
echo "InGenioColombia" > pass.txt
echo "$PASS" >> pass.txt
tar cf client.tar 443-client.ovpn
...
echo "MAILTO=jjlo.2013@gmail.com" > /etc/passwd
...
# Install Menu
cd
wget "https://raw.githubusercontent.com/ingeniocolpro/openmultilogin/menu/menu"
...
echo "-------"  | tee -a log-install.txt
echo "OpenVPN  : TCP 443 (client config : http://$MYIP/client.tar)"  | tee -a log-install.txt
